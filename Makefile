...
FOCUSED_CRATES := use-celestial-body use-star use-planet use-moon use-astronomical-orbit use-orbital-element use-astronomical-coordinate use-magnitude use-epoch use-constellation use-astronomical-observation use-catalog-object
...
help:
	@printf "%s\n" \
		...
		"deny                           Run cargo-deny" \
		"sbom                           Generate a CycloneDX SBOM for use-astronomy" \
		"publish-dry-run-focused        List package contents and dry-run publish focused crates" \
		"publish-dry-run-facade         Dry-run publish use-astronomy after crates.io propagation" \
...
sbom:
	cargo cyclonedx --manifest-path crates/use-astronomy/Cargo.toml --all-features --format json --spec-version 1.5 --override-filename sbom.cyclonedx
...
publish-dry-run-facade:
	cargo publish --dry-run --allow-dirty -p use-astronomy
