...
Use this checklist when adding a new focused crate or expanding the `use-astronomy` facade.
...
[package]
name = "use-example"
description = "Primitive astronomy example vocabulary for RustUse"
publish = true
...
keywords = ["astronomy", "celestial", "vocabulary", "rustuse"]
...
- Keep the crate out of simulation, rendering, telescope control, catalog fetching, and analysis domains unless a future set explicitly owns that scope.
...
If a new focused crate should be available through `use-astronomy`, also update:

- root `Cargo.toml` workspace members, workspace dependencies, and first-publish metadata
- `crates/use-astronomy/Cargo.toml` dependencies
- `crates/use-astronomy/src/lib.rs` module aliases
- `crates/use-astronomy/README.md`
- root `README.md`
...
- Keep examples descriptive. They should not fetch catalogs, simulate motion, render sky maps, control telescopes, or call external services.
