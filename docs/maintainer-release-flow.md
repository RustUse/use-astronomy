...
- The shared root `CHANGELOG.md` is generated through the `use-astronomy` package entry and includes focused-crate commits.
...
- Configure crates.io Trusted Publishing for every published crate with repository owner `RustUse`, repository name `use-astronomy`, and workflow filename `release-plz-release.yml`.
...
1. Confirm the intended first-wave publishable crates are still `use-celestial-body`, `use-star`, `use-planet`, `use-moon`, `use-astronomical-orbit`, `use-orbital-element`, `use-astronomical-coordinate`, `use-magnitude`, `use-epoch`, `use-constellation`, `use-astronomical-observation`, `use-catalog-object`, and `use-astronomy`.
...
5. Run `cargo publish --dry-run -p use-astronomy` or the manual `Facade Publish Readiness` workflow.
