...
RustUse/use-astronomy keeps root workspace metadata at `publish = false` by default, while the current focused crate manifests and facade manifest opt in with `publish = true`. The release task is to verify that only the intended crates remain publishable.
...
1. `use-celestial-body`
2. `use-star`
3. `use-planet`
4. `use-moon`
5. `use-astronomical-orbit`
6. `use-orbital-element`
7. `use-astronomical-coordinate`
8. `use-magnitude`
9. `use-epoch`
10. `use-constellation`
11. `use-astronomical-observation`
12. `use-catalog-object`
...
- keeps `crates/use-astronomy/Cargo.toml` at `publish = true`
...
- `.github/workflows/facade-publish-readiness.yml` is a manual post-publication check that dry-runs `use-astronomy` after the focused crates are live on crates.io.
...
- The root `CHANGELOG.md` remains the shared changelog and is updated through the `use-astronomy` package entry, including focused-crate commits.
...
- Configure crates.io Trusted Publishing for each published crate with repository owner `RustUse`, repository name `use-astronomy`, and workflow filename `release-plz-release.yml`.
...
5. Run the facade publish readiness workflow or `cargo publish --dry-run --allow-dirty -p use-astronomy`.
6. Publish `use-astronomy`.
...
9. Confirm focused crates plus `use-astronomy` are the only intentionally publishable crates.
